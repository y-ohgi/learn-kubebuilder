...
```sh
make docker-build docker-push IMG=<some-registry>/learn-kubebuilder:tag
```
...
```sh
make deploy IMG=<some-registry>/learn-kubebuilder:tag
```
...
```sh
make build-installer IMG=<some-registry>/learn-kubebuilder:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/learn-kubebuilder/<tag or branch>/dist/install.yaml
```
